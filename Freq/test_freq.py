...
net = Model_freq(hidden1 = 128, classes = 23).to(device)
net.load_state_dict(torch.load('...'))

#net = VI(hidden1 = 128,hidden2 = 64, classes = 23).to(device)
#net.load_state_dict(torch.load('model_iq_signal/vi_23classes_30102023_data2018_64last_sumall_oqpsk_pi5_freq400_sample_nbr5_epoch_5.pt'))
...
for sequence in seq_len:
    ...
    for data, target, index in loader:
        ...
        #test_Y_hat = net(test_X_i.to(device))[:, -1, :]
        test_Y_hat = out_average(net, test_X_i, num_test=20)
        #test_Y_hat = net(test_X_i.to(device))
        # estimate classes
